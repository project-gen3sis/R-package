...
```{r eval=T, fig.width=7, fig.height=3.5, message=T, echo=F, warning=T, results='hide'}
library(raster)
...
After having run the simulation we can load and plot the species at different time steps. Here, we will load them at the timesteps 300, 150 and 0 which correspond to 150, 125 and 100 Ma, respectively.

```{r eval=T, echo=T, fig.width=7, fig.height=5}
timesteps <- c(300, 150, 0) #since we have 6 steps between each million year and our landscape ends at 100Ma
par(mfrow=c(1,3))
for(i in timesteps){
