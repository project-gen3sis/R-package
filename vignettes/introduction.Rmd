...
```{r eval=TRUE, fig.width=7, fig.height=6, fig.retina=2, message=F, results='hide'}
#do linear model between temperature and aridity
lm <- glm(prec ~ temp, data=landscape_t0)
cor(landscape_t0$prec, landscape_t0$temp)

#Plot the response curve
data_plot <- data.frame(cbind(landscape_t0$temp, predict(lm, type = "response")))
sorted <- na.omit(data_plot[order(data_plot[,1], decreasing = FALSE),])
data_plot <- data.frame(cbind(sorted[,1],sorted[,2]))
n <- paste0('observations (n = ', length(landscape_t0$temp), ')')
plot(data_plot[,1],data_plot[,2], xlab="Temperature", ylab="Aridity", frame.plot=F, type="l", col='black', lwd=2)
...
Indeed aridity is correlated with temperature. Thus let'sinvestigate the association of aridity and species richness.
