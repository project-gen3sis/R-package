...
```{r eval=TRUE, fig.width=5, fig.height=5, message=F}
library(raster)
...
plot(data_plot[,1],data_plot[,2], xlab="Temperature", ylab="Species richness", frame.plot=F, type="l", col='blue', lwd=2)
...
legend(5,20, col=c(rgb(0.5,0.5,0.5,0.4), 'blue'), legend=c('obersvations', 'model fit'), pch=c(16, NA), lty=c(NA, 1), lwd=c(NA, 2))
```
...
legend(0,10, col=c(rgb(0.5,0.5,0.5,0.4), 'blue'), legend=c('obersvations', 'model fit'), pch=c(16, NA), lty=c(NA, 1), lwd=c(NA, 2))
```
...
sort1 <- na.omit(data_plot[order(data_plot[,1], decreasing = FALSE),])
data_plot <- data.frame(cbind(sort1[,1],sort1[,2]))
plot(data_plot[,1],data_plot[,2], xlab="Area", ylab="Species richness", frame.plot=F, type="l", col='blue', lwd=2)
#Plot the presences in red and absences in blue
