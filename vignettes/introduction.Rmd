...
### Landscape
The **landscape** is the environment of the virtual planet (e.g. temperature and precipitation data that changes spatially and temporally). 
...
```{r eval=FALSE, fig.width=7, fig.height=5.5, message=F, echo=F, warning=F, results='hide'}
   library(raster)
   ...
   title(main=expression('Precipitation'), line=1, outer=T, cex.main=2)
...
<!--
![This figure shows temperature (top), precipitation (middle) and cell size (bottom) of the input landscape of the sample world in the beginning of the simulation (150Ma), in the middle (125Ma) and in the end of the simulation (100Ma).](../inst/extdata/InputRasters/WorldCenter/landscape_area.png){width=105%}
-->


```{r  out.width = "100%", fig.cap='This figure shows temperature (top), precipitation (middle) and cell size (bottom) of the input landscape of the sample world in the beginning of the simulation (150Ma), in the middle (125Ma) and in the end of the simulation (100Ma).', echo=F}
...
```{r eval=T, echo=F}
species <- readRDS(file.path(datapath, "output/config_worldcenter/sgen3sis.rds"))
```
...
Analog we can also look at the association of precipitation and species richness.
...
sorted <- na.omit(data_plot[order(data_plot[,1], decreasing = FALSE),])
data_plot <- data.frame(cbind(sorted[,1],sorted[,2]))
plot(data_plot[,1],data_plot[,2], xlab="Precipitation", ylab="Species richness", frame.plot=F, type="l", col='blue', lwd=2)
...
Dry regions seem to harbour a higher number of species, as shown in the plot above and indicated by the negative correlation of `r round(cor(landscape_t0$prec, landscape_t0$rich), 4)` between precipitation and species richness.
