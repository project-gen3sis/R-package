...
All the data we will use here (i.e. the **landscape** and the **configuration** object) is available with the package, so you only will need a path to the data. For more information about these input data, please check the associated metadata.This is stored in the *METADATA.txt* file inside the landscape folder and,  for the configuration, as a comment in the beginning of the config R script.
