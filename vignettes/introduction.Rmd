...
All the data we will use (i.e. the **landscape** and the **configuration** object) is available with the package, you can just get the path to the data inside the package. To access this dataset, you first need to define the path to the data contained inside the package.
...
### Landscape
The **landscape** is the virtual environment (e.g. temperature and aridity data that might be spatially and temporally dynamic). 

**NOTE** that the landscape contains several files. The .rds file contains the landscape changing over time and the distances folder stores some data to inform how the landscape is connected. In this example you are using an already ready-to-run landscape. To see how to create a landscape object, see [design_landscape.html](this vignette).
...
By looking at the model fit of the generalized linear model we can see that the relationship between the two variables is rather hump-shaped, indicating that species richness highest at intermediate temperature levels. First the higher richness found above 10° is consistent with the ecological rule that we imposed. At higher temperature, we have a higher energy and capacity facilitating coexist. This is consistent with our model expectations. Moreover, we can observe a large spread of the points at high temperature. This is likely because high temperature regions frequently have high aridity. This is why we need to also explore aridity.

For this we first fit a linear model between temperature and aridity and then plot aridity~temperature, similarly to how it was done before.
```{r eval=TRUE, echo=TRUE}
linear_model <- lm(prec ~ temp, data=landscape_t0)
cor(landscape_t0$prec, landscape_t0$temp)
```

```{r eval=TRUE,echo=FALSE, fig.width=7, fig.height=6, fig.retina=2, message=F, results='hide'}
#Plot the response curve
data_plot <- data.frame(cbind(landscape_t0$temp, predict(linear_model, type = "response")))
data_plot <- na.omit(data_plot[order(data_plot[,1], decreasing = FALSE),])
n <- paste0('observations (n = ', length(landscape_t0$temp), ')')
plot(data_plot[,1],data_plot[,2], xlab="Temperature [\u00B0C]", ylab="Aridity [index]", frame.plot=F, type="l", col='black', lwd=2)
points( landscape_t0$temp,landscape_t0$prec, col=rgb(0.5,0.5,0.5, alpha=0.4), pch=16) 
legend("topleft", col=c(rgb(0.5,0.5,0.5,0.4), 'red'), legend=c(n, 'model fit'), pch=c(16, NA), lty=c(NA, 1), lwd=c(NA, 2), bty='n')
```

Indeed aridity is correlated with temperature. Thus let's investigate the association of aridity and species richness.
