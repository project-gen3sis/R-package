...
The second argument we need to define is a cost function. This function should define how connected and reachabel grid cells are via dispersal. For example, suitable terrestrial cells should pose no cost for dispersal, but dispersal over water grid cells should increase the cost of dispersal. However, in our example all grid cells are suitable and contiguous and the distance unit is automatically in grid cell as we do not provide a coordinate system. Hence, we can define a simple cost function in which the dispersal is not penalized (i.e. equals to 1, in meters). 
For real landscapes, the distance unit is in kilometers (km). Because of this create_input() scales from meters given in the cost function to kilometers by multiplying the cost by a factor of 1/1000. This only works if a coordinate system is set for the input rasters. In the case of hypothetical landscapes, as in our example, geographic distance is measured in units of grid cells. Hence, we need to add a correction factor of 1/1000 already in the cost function so the distance matrices are in grid cell unit.  

```{r eval=TRUE}
my_cost_function <- function(x, h_x, y, h_y){
  if(all(h_x, h_y)){
    return(1*(1/1000)) #scaling to 1/1000 to have distance matrix in grid cell units 
  } else {
    return(1*(1/1000)) #scaling to 1/1000 to have distance matrix in grid cell units 
  }
}
