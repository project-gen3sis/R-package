...
temp[[1]]
lapply(seq(temp), function(x){names(temp[[x]])<-paste("X",x,"Ma",sep="")})
temp[[1]]

names(temp[[1]])<-paste("X",x,"Ma",sep="")
#stringtimestepsnames
...
my_cost_function <- function(x, h_x, y, h_y){
  if(all(h_x, h_y)){
    return(1) #scaling to 1/100 to get to the 1 eucledean distance
  } else {
    return(1) #scaling to 1/100 to get to the 1 eucledean distance
  }
}
